...
type StoreUpdateHandler = (state: any, key: string, value: any) => void

interface Dictionary {
	[key: string]: any
}

interface Store extends Dictionary {
	sub (sub: StoreUpdateHandler): (() => void)
}

function createStore (initialState: Dictionary): Store {
	let subs: Set<StoreUpdateHandler> = new Set()
	let state: Dictionary = {}
	let store: Store = Object.defineProperty({}, "sub", {
		enumerable: false,
		value (fn: StoreUpdateHandler) {
...
interface GlobalState extends Store {
	mode: number,
	shift: boolean,
	metronome: boolean,
	playing: boolean,
...
// @ts-ignore
let state: GlobalState = createStore({
	mode: modes.perform,
	shift: false,
	metronome: false,
	playing: false,
...
declare var masterTrack: Track
declare var cursorTrack: CursorTrack
declare var trackBank: TrackBank
declare var cursorDevice: CursorDevice
declare var userControls: UserControlBank
declare var surface: HardwareSurface
...
type MidiKnobHandler = (data: number) => void
type MidiButtonHandler = () => void
type MidiHandler = MidiButtonHandler | MidiKnobHandler

interface MidiHandlers {
	[key: number]: MidiHandler
}

let midiHandlers: MidiHandlers = {
	[op1.cc.play] () {
		if (state.shift) {
...
function getKnobIncArgs (data: number) {
	return {
...
let fancyHandlers: MidiHandlers = {
	[op1.cc.blue] (data) {
...
function onMidi(_status: number, data1: number, data2: number) {
